...
def run(iteration):
	...
	summary_file = TEMP_PATH + '/TF_' + _0_Preprocess.RND_ID + '_summary.txt'
	...
	for disease in dd:
		...
		auc_output = TEMP_PATH + '/TF_'+_0_Preprocess.RND_ID + '_' + disease + '_auc.txt'
...
def __loadAnnotationFile(annotation_file):
	...
	while(True):
		...
		for s in lines:
			...
			string_tf_id = id_box.getSTRING_ID_of(tf_id)
			string_target_id = id_box.getSTRING_ID_of(target_id)
			
			
			if string_tf_id is not None and string_target_id is not None:
				# add an TF
				if not string_tf_id in r:
					r[string_tf_id] = []
				if not string_target_id in r[string_tf_id]:
					r[string_tf_id].append(string_target_id)
...
def calculate_mpi(args):
	...
	description, tf_box, tf_box_rev, total_genes, mod_pos, output, per, TEMP_PATH, RND_ID, queue = args
	...
	if P_VALUE == True:
		...
		if CPU <= 1:
			...
			for i in range(steps):
				args = [description, tf_box, tf_box_rev, total_genes, mod_pos, output, per, TEMP_PATH, RND_ID]
			...
			ret_files = MULTIPROCESS.runMultiprocesses(thread_box, max_cpu = steps, sleep_interval_seconds=10,
			                                   title='[TF calculation]', cutoff=CPU )
...
def runFinal():
	...
	if len(predict_disease_specific_modifiers) == 0:
		...
		print("[_3_TF.py] 예측할 disease-specific modifiers가 없음 -> 건너뜀")
		return
	...
	for disease in mods:
		...
		if not disease in predict_disease_specific_modifiers:
			print('[_3_TF.py] disease-specific modifiers 예측 안하고 건너뜀: ', disease)
			continue
		...
		#output = OUTPUT_PATH + '/TF_' + disease+'_' +  str(ITERATION) + '_' + RND_ID + '.txt'
		output = TEMP_PATH + '/TF_' + disease + '_' + str(ITERATION) + '_' + RND_ID + '.txt'
