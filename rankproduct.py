...
def __saveRscript(code):

	temp_file = 'R:/TEMP/' + MyUtil.getRandomString(20) + '.R'
	f=open(temp_file,'w')
	f.write(code)
	f.close()
...
def __execute_R(fname):

	logfile = 'R:/TEMP/log_file_' + MyUtil.getRandomString(10) + '.Rout.txt'
	cmd = 'R CMD BATCH ' + fname + ' ' + logfile
...
def calculate_rankproduct_p_value ( rho_list, n, k, delta = 'geometric'):
	'''

	:param rho_list: 정수형 rank 값들을 모두 곱한 것
	:param n: rank에 포함된 것들의 전체 수 (0-n개)
	:param k: 전체 몇개의 replica가 존재하는가 (k개 이면, rankproduct=r1*r2*r3...*rk가 됨.
	:param delta: (upper, lower, geometric) 3개 중 하나여야 함.
	:return: p-value의 리스트
	'''
	...
	for i in rho_list:
		txt = 'rho <- c(' + str(i) + ')' + '\n' + \
			"print( rankprodbounds(rho, n, k, Delta=c('geometric') ) )"
		code += '\n' + txt


	temp_file = __saveRscript(code)
	ret_file = __execute_R(temp_file)
	ret_list = __parse(ret_file)
...
if __name__ == '__main__':
	...
	n = 100
	k = 10
	print calculate_rankproduct_p_value(rho, n, k)
